...
generateMoves(_,[],_,[]):-!.
generateMoves(node(Cost,[],State,ListActionsMoment),[Action|OtherActions],PerformedMoves,[node(NewCost,[],NewState,[Action|ListActionsMoment])|ListMoves]):-
    trasform(Action,State,NewState),
    NewCost is Cost + 1,
    \+member(NewState,PerformedMoves),!,
    generateMoves(node(Cost,[],State,ListActionsMoment),OtherActions,PerformedMoves,ListMoves).
...
orderMoves(NewListMoves,GoalsList,ResidualMoves,[BestMove|OtherMoves]):-
	applyHeuristicGoals(NewListMoves,GoalsList,ListMovesHeuristic),
	append(ResidualMoves,ListMovesHeuristic,HeuristicList),
...
applyHeuristic([],_,_,[]):-!.
applyHeuristic([node(Cost,_,pos(Row,Column),ListActionsMoment)|ResidualMoves],FinalRow,FinalColumn,[node(Heuristic,NextCost,pos(Row,Column),ListActionsMoment)|ListMovesHeuristic]):-
	calculateHeuristic(Row,Column,FinalRow,FinalColumn,NextCost),
	Heuristic is NextCost + Cost,
	applyHeuristic(ResidualMoves,FinalRow,FinalColumn,ListMovesHeuristic).
...
calculateHeuristic(Row,Column,FinalRow,FinalColumn,Distance):-
	...
	Distance is max(DifferenceRow,DifferenceColumn).
